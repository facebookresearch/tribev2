...
For notebook or Colab usage, it is helpful to run an environment check cell first. If imports fail after installing dependencies inside the notebook runtime, restart the kernel/runtime and rerun the checks before continuing. Some notebook features also depend on optional plotting packages, and audio/video preprocessing may require `ffmpeg` to be available on the system.
